                               ...
                               uint16_t length)
{
  ...
  /* Instrumentation: Log control request details */
  USBH_UsrLog("USBH_CtlReq: State=%d, Status=%d, Length=%d", phost->RequestState, status, length);
  ...
  switch (phost->RequestState)
  {
    case CMD_SEND:
      ...
      USBH_UsrLog("CMD_SEND: Setup transfer initiated.");
    ...
    case CMD_WAIT:
      ...
      USBH_UsrLog("CMD_WAIT: Status=%d", status);
      if ((status == USBH_OK) || (status == USBH_NOT_SUPPORTED))
      {
        /* Transaction completed, move control state to idle */
        phost->RequestState = CMD_SEND;
        phost->Control.state = CTRL_IDLE;
        USBH_UsrLog("CMD_WAIT: Transaction completed.");
      }
      else if (status == USBH_FAIL)
      {
        /* Failure Mode */
        phost->RequestState = CMD_SEND;
        USBH_ErrLog("CMD_WAIT: Transaction failed.");
      }
      else
      {
        /* .. */
    ...
    default:
      USBH_ErrLog("USBH_CtlReq: Unknown state.");
      break;
  }
  return status;
}
