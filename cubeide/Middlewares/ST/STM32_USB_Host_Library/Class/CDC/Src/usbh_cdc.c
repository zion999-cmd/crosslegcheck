...
/* Runtime diagnostic hooks (defined in user usb_host.c) */
...
static USBH_StatusTypeDef USBH_CDC_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  /* Some CDC devices set the communication interface protocol to 0x00
    instead of the COMMON_AT_COMMAND (0x01). Accept any protocol here
    (0xFF) so that devices that use protocol 0x00 are still detected. */
  interface = USBH_FindInterface(phost, COMMUNICATION_INTERFACE_CLASS_CODE,
                        ABSTRACT_CONTROL_MODEL, 0xFFU);

  /* Debug: report which interface/class/subclass/protocol were present
    (helps when analysing remote device descriptors). */
  USBH_DbgLog("Looking for CDC Communication interface (class=0x%02x, subclass=0x%02x)", COMMUNICATION_INTERFACE_CLASS_CODE, ABSTRACT_CONTROL_MODEL);
  ...
  /* Log endpoint information for debugging: notification endpoint and data
   * IN/OUT endpoints with their max packet sizes. This helps verify the host
   * selected the correct endpoints for the device. */
  /* Use category-based logging so payload logs can be gated separately. */
  USBH_CAT_LOG(USB_LOG_CAT_CDC, "CDC EPs: Notif=0x%02X(%u) In=0x%02X(%u) Out=0x%02X(%u)",
              CDC_Handle->CommItf.NotifEp, (unsigned int)CDC_Handle->CommItf.NotifEpSize,
              CDC_Handle->DataItf.InEp, (unsigned int)CDC_Handle->DataItf.InEpSize,
              CDC_Handle->DataItf.OutEp, (unsigned int)CDC_Handle->DataItf.OutEpSize);
...
static USBH_StatusTypeDef USBH_CDC_ClassRequest(USBH_HandleTypeDef *phost)
{
  ...
  snprintf(usb_last_event, sizeof(usb_last_event), "CDC: ClassRequest(start)");
  usb_last_event_time_ms = HAL_GetTick();
  status = GetLineCoding(phost, &CDC_Handle->LineCoding);
  if (status == USBH_OK)
  {
    snprintf(usb_last_event, sizeof(usb_last_event), "CDC: ClassRequest(ok)");
    usb_last_event_time_ms = HAL_GetTick();
...
static USBH_StatusTypeDef GetLineCoding(USBH_HandleTypeDef *phost, CDC_LineCodingTypeDef *linecoding)
{
  ...
  /* Diagnostic: mark that a GetLineCoding control request is being started */
  snprintf(usb_last_event, sizeof(usb_last_event), "CDC: GET_LINE_CODING(start)");
  usb_last_event_time_ms = HAL_GetTick();
  ...
  USBH_StatusTypeDef ret = USBH_CtlReq(phost, linecoding->Array, LINE_CODING_STRUCTURE_SIZE);
  if (ret == USBH_OK) {
    snprintf(usb_last_event, sizeof(usb_last_event), "CDC: GET_LINE_CODING(ok)");
    usb_last_event_time_ms = HAL_GetTick();
  }
  return ret;
}
